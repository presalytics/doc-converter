...
docker run -d -t --network=host -p ${DocConverterServer}:${DocConverterPort}:${DocConverterPort} -p ${DocConverterServer}:${DocConverterRemoteDebugPort}:${DocConverterRemoteDebugPort} --name doc_converter --env-file ./../../.env doc_converter
